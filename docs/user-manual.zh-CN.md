...
### 4.4 EFMI Merged 分割操作

**分割操作**把多个原始 EFMI Component 的边界投射到一个完整身体或衣服，再生成可导出的 Component 副本。该功能只支持 `Merged`，并要求对象源目录中存在匹配的 `VertexGroupMap.json`。

1. 在 EFMI 导出设置中选择组件集合和对象源目录。
2. 选中需要合并的原始 Component，例如 C1、C2、C8、C9。
3. 在 **分割操作** 中点击 **创建分区参考体**。
4. 使用现有 **权重工具**，把参考体权重传到完整 Master。
5. 设置 **分区参考体** 和 **完整 Master**，点击 **投射分区并拆分**。
6. 检查 C1、C2 等子集合中的生成对象；完整 Master 会保留。

原始 Component 只会隐藏，不会移动、改名或删除。Velo 会明确阻止这些源对象、参考体和完整 Master 进入 EFMI 导出，不依赖 **忽略隐藏对象**。

分割边界只沿 Master 的现有网格边。边界两侧复制相同的位置、ShapeKey、UV、corner normal 和合法化权重。若单个顶点需要丢弃超过 10% 权重，操作停止并选择问题顶点；5%-10% 会报告警告。点击 **选择模糊面**可以检查投射置信度低的边界面；点击 **恢复原始部件**可删除生成结果并恢复源对象。
