...
def _build_reference_from_legacy(context, legacy_obj, partition_id: str, authoring, previous_reference=None):
    ...
    reference[ROUTES_KEY] = legacy._routes_json(routes)
...
def _resolve_merge_map(settings, routes, config, body):
    ...
    return legacy._resolve_component_merges(settings, routes, config)
...
def _split_one(
    ...
    *,
    ...
    if merge_map is None:
        merge_map = legacy._resolve_component_merges(settings, routes, config)
    legacy._apply_component_merges(projection, merge_map)
...
def _sync_impl(context):
    ...
    config = legacy._load_configuration(context.scene)
    _validate_reference(reference)
    ...
    routes = legacy._load_routes(reference, config)
    palettes = legacy._palettes_for_routes(config, routes)
